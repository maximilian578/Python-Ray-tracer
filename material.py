...
class Material(object):
    ...
    def litColour(
        ...
        for light in [x for x in lights if x is not None]:
            if self.shininess:
                H = unit(light.vector + viewVector)
                colour += (max(0, normal.dot(light.vector)) * diffcol + normal.dot(H)
                           ** self.shininess * self.specularColour) * light.colour
